...
void docommand(struct command com, int s)
{
    /*
    printf("Received %d command from %d\n", (int )ntohl(com.command), s);
    */

    switch(ntohl(com.command))
    {
	case(COM_STOP):
	    getsolution(com); break;
	case(COM_INIT):
	    inituser(s); break;
        case(COM_GET):
	    sendpasswords(s); break;
        case(COM_STAT):
	    sendstatus(s); break;
    }
}
	...
	if( (hits=select(MAXCONS, &fdset, NULL, NULL, NULL)) > 0)
	{
	    ...
	    for(i=s+1; (i<MAXCONS && hits>0); i++)
	    {
		    ...
		    hits--;
	            ...
	            {
			docommand(com, i);
			if(ntohl(com.command)==COM_STOP) done=1;
	            }
		    else
		    {
			pipe_del(i);
