 ...
 *
 * $Orig: mymalloc.c,v 1.1 1998/06/22 00:07:32 dustin Exp $
 * $Id: mymalloc.c,v 1.1 2001/02/24 12:55:56 dustin Exp $
 */
...
static int _getmemindex(void *p)
{
    return( (int)p%NUM_MEMBUCKETS);
}

static void _register_mem(void *p, size_t size, char *file, int line)
{
    struct memories *m, *c;
    int index;
...
static void _unregister_mem(void *p)
{
...
void _mdebug_dump(void)
{
    ...
    max=0; min=INT_MAX; avg=0; empty=0;
    for(i=0; i<NUM_MEMBUCKETS; i++) {
	if(_mem_stats[i]==0)
	    empty++;
	if(_mem_stats[i]>max)
