...
function kittyCode() {
	...
	if(document.form.kittyinput.value.length > 0) {
		...
		document.form.libID.value="";
		document.form.libBook.value="";
		...
		// document.upcform.upc.value="";
			...
			document.form.isbn.value = upcISBNmap(code.substr(0,6)) 						+code.substr(12,5);
			document.form.price.value=toPrice(code.substr(7,4));
			var country="";
			if (code.substr(6,1) ==5) {country="USA";
			} else if (code.substr(6,1) ==6) {country="CANADA";
			} else if (code.substr(6,1) ==9) {country="OTHER"; }
			document.form.country.value=country;
		}

            if( a.test(type) | b.test(type) | c.test(type) | a1.test(type)) {
                document.form.isbn.value = checkISBN(document.form.isbn.value);
		    if (document.form.ISBNAutoLookup.checked) formHandler();

		}
		var d = /CC/;
		if (d.test(type)) {     // we got a cuecat code - all CC! in RS cat.
			document.form.cuecat.value="CC " + decodeCC(type[2]) + 						decodeCC(code);
                  document.form.code.value="";
		}

		if (((type == "UPA") | (type=="UPE") )
                   & (document.form.upcAutoLookup.checked)) {
		 window.location.href="http://www.upcdatabase.com/item.pl?upc="+code;
		}
...
function checkISBN(str) {
   ...
   result = sum % 11;
str = str.substr(0,9);
   if (result==10) {str+="X";} else {str+= result;}
   return str;
}
...
function upcHandler(){
	if ((document.form.type.value == "UPA") |
		...
		window.location.href="http://www.upcdatabase.com/item.pl?upc="+document.form.code.value;
	}
}
